...
class Bottleneck34(nn.Module):
    ...
    def __init__(
        ...
        super(Bottleneck34, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        width = int(planes * (base_width / 64.0)) * groups
        # 利用3x3卷积进行特征提取
        self.conv1 = conv3x3(width, width, stride, groups, dilation)
...
def resnet34(pretrained=False, **kwargs):
    model = ResNet34(BasicBlock, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(
            model_zoo.load_url(
