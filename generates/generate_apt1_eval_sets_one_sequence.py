...
mat = io.loadmat('./iou_apt1_1.mat')
mat = mat['iou_non_empty_matrix']
train_iou_mat = mat[:715, :715]
test_iou_mat = mat[715:,:715]
test_iou_mat_up = mat[:715, 715:]
# test_iou_mat_up = mat[715:,715:]
...
# test_pose_list = test_pose_list[715:]
test_pose_list = np.array(test_pose_list)[test_list]
...
train_pointcloud_dir = root_dir + '/pointcloud_4096/'
test_pointcloud_dir = root_dir + '/pointcloud_4096/'
train_pointcloud_list = os.listdir(train_pointcloud_dir)
train_pointcloud_list.sort()
print(len(train_pointcloud_list))
# train_pointcloud_list = train_pointcloud_list[:715]
train_pointcloud_list = np.array(train_pointcloud_list)[train_list]
test_pointcloud_list = os.listdir(test_pointcloud_dir)
test_pointcloud_list.sort()
# train_pointcloud_list = test_pointcloud_list
# test_pointcloud_list = test_pointcloud_list[715:]
test_pointcloud_list = np.array(test_pointcloud_list)[test_list]

test_pickle = pickle.load(open('/home/graham/datasets/apt1/kitchen/pickle/apt1_kitchen_test_random_iou.pickle', 'rb'))
train_pickle = pickle.load(open('/home/graham/datasets/apt1/kitchen/pickle/apt1_kitchen_train_random_iou.pickle', 'rb'))
# test_pickle = pickle.load(open('/home/graham/datasets/fire/train/pickle/fire_test_random_iou.pickle', 'rb'))
# train_pickle = pickle.load(open('/home/graham/datasets/fire/train/pickle/fire_train_random_iou.pickle', 'rb'))
...
def construct_query_and_database_sets():
    ...
    for i in range(len(database_sets)):
        for j in range(len(test_sets)):
            for key in range(len(test_sets[j].keys())):
                ...
                #     if count > 24:
                #         break
                #     if abs(test_iou_mat[key][ii]  - test_iou_mat_up[ii][key]) < 0.1 and test_iou_mat[key][ii] + test_iou_mat_up[ii][key] > 0.12:
                #         count += 1
                        # ind.append(ii)
            ...
            for key in range(len(database_sets[i].keys())):
                ...
                #     if count > 24:
                #         break
                #     if abs(train_iou_mat[key][ii]  - train_iou_mat[ii][key]) < 0.1 and train_iou_mat[key][ii] + train_iou_mat[ii][key] > 0.12:
                #         count += 1
                #         ind.append(ii)
                        # print(iou_mat[key][ii])
    ...
    output_to_file(database_sets, root_dir + '/pickle',
                   'apt1_kitchen_evaluation_database.pickle')
    output_to_file(test_sets, root_dir + '/pickle',
                   'apt1_kitchen_evaluation_query.pickle')
